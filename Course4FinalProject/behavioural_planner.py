...
class BehaviouralPlanner:
    ...
    def get_goal_index(self, waypoints, ego_state, closest_len, closest_index):
        ...
        while wp_index < len(waypoints) - 1:
            wp_index += 1
            wp_dist_delta_vector = [waypoints[wp_index][0] - waypoints[wp_index - 1][0],
                                    waypoints[wp_index][1] - waypoints[wp_index - 1][1]]
            wp_distance = np.linalg.norm(wp_dist_delta_vector)
            arc_length += wp_distance
            if arc_length > self._lookahead:
                break
        # ------------------------------------------------------------------
...
def get_closest_index(waypoints, ego_state):
    ...
    for i in range(len(waypoints)):
        closest_wp_delta_vector = [waypoints[i][0] - ego_state[0],
                                   waypoints[i][1] - ego_state[1]]
        wp_distance = np.linalg.norm(closest_wp_delta_vector)
        if (wp_distance < closest_len):
            closest_len = wp_distance
            closest_index = i
    # ------------------------------------------------------------------
